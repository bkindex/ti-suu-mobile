(function(publish) {
	...
	var VERSION		= "1.01";
	var RS_VERSION	= "2.0";
	var EN_ALPHABET	= "([A-Za-z]+?)";
	// var VI_ALPHABET = "([A-Za-záàảãạâấầẩẫậăắằẳẵặđéèẻẽẹêếềểễệíìỉĩịóòỏõọôốồổỗộơớờởỡợúùủũụưứừửữựýỳỷỹỵÁÀẢÃẠÂẤẦẨẪẬĂẮẰẲẴẶĐÉÈẺẼẸÊẾỀỂỄỆÍÌỈĨỊÓÒỎÕỌÔỐỒỔỖỘƠỚỜỞỠỢÚÙỦŨỤƯỨỪỬỮỰÝỲỶỸỴ]+?)";
	// var VI_WORD_SUBSTITUTE = "([^A-Za-z0-9áàảãạâấầẩẫậăắằẳẵặđéèẻẽẹêếềểễệíìỉĩịóòỏõọôốồổỗộơớờởỡợúùủũụưứừửữựýỳỷỹỵÁÀẢÃẠÂẤẦẨẪẬĂẮẰẲẴẶĐÉÈẺẼẸÊẾỀỂỄỆÍÌỈĨỊÓÒỎÕỌÔỐỒỔỖỘƠỚỜỞỠỢÚÙỦŨỤƯỨỪỬỮỰÝỲỶỸỴ]+?)";
	// var VI_ALPHABET_IGNORE = "(?:[A-Za-záàảãạâấầẩẫậăắằẳẵặđéèẻẽẹêếềểễệíìỉĩịóòỏõọôốồổỗộơớờởỡợúùủũụưứừửữựýỳỷỹỵÁÀẢÃẠÂẤẦẨẪẬĂẮẰẲẴẶĐÉÈẺẼẸÊẾỀỂỄỆÍÌỈĨỊÓÒỎÕỌÔỐỒỔỖỘƠỚỜỞỠỢÚÙỦŨỤƯỨỪỬỮỰÝỲỶỸỴ]+?)";
	// var RE_ALPHABET_SUBSTITUTE = /[^A-Za-z0-9áàảãạâấầẩẫậăắằẳẵặđéèẻẽẹêếềểễệíìỉĩịóòỏõọôốồổỗộơớờởỡợúùủũụưứừửữựýỳỷỹỵÁÀẢÃẠÂẤẦẨẪẬĂẮẰẲẴẶĐÉÈẺẼẸÊẾỀỂỄỆÍÌỈĨỊÓÒỎÕỌÔỐỒỔỖỘƠỚỜỞỠỢÚÙỦŨỤƯỨỪỬỮỰÝỲỶỸỴ ]/g
	var VI_ALPHABET = "([a-záàảãạâấầẩẫậăắằẳẵặđéèẻẽẹêếềểễệíìỉĩịóòỏõọôốồổỗộơớờởỡợúùủũụưứừửữựýỳỷỹỵ]+?)";
	var VI_WORD_SUBSTITUTE = "([^a-z0-9áàảãạâấầẩẫậăắằẳẵặđéèẻẽẹêếềểễệíìỉĩịóòỏõọôốồổỗộơớờởỡợúùủũụưứừửữựýỳỷỹỵ]+?)";
	var VI_ALPHABET_IGNORE = "(?:[a-záàảãạâấầẩẫậăắằẳẵặđéèẻẽẹêếềểễệíìỉĩịóòỏõọôốồổỗộơớờởỡợúùủũụưứừửữựýỳỷỹỵ]+?)";
	var RE_ALPHABET_SUBSTITUTE = /[^a-z0-9áàảãạâấầẩẫậăắằẳẵặđéèẻẽẹêếềểễệíìỉĩịóòỏõọôốồổỗộơớờởỡợúùủũụưứừửữựýỳỷỹỵ ]/g
	var RE_TRIGGER 	= /[^(|)\[\]*_#@{}<>=\sa-z0-9áàảãạâấầẩẫậăắằẳẵặđéèẻẽẹêếềểễệíìỉĩịóòỏõọôốồổỗộơớờởỡợúùủũụưứừửữựýỳỷỹỵ]/
		...
		$.ajax({
			...
			error: function(xhr, textStatus, errorThrown) {
				...
				if (typeof(on_error) == "function") {
					on_error.call(RS,loadcount,textStatus);
				}
			},
		});
	};
	...
	RiveScript.prototype._node_load_file = function (loadcount, file, on_success, on_error) {
		...
		this._node.fs.readFile(file, function (err, data) {
			if (err) {
				if (typeof(on_error) == "function") {
					on_error.call(RS,loadcount,err);
				} else {
					RS.warn(err);
				}
				return;
...
//			this.say("Line: " + line + " (topic: " + topic + ") incomment: " + inobj);
			...
			switch(cmd) {
				case '!': // ! DEFINE
					...
					// var left   = this._strip(halves[0]).split(" ", 2);  // name có nhiều hơn 1 word
	...
	RiveScript.prototype.checkSyntax = function (cmd, line) {
		...
		if (cmd == '!') {
			...
			// if (line.match(/[^a-z0-9(|)\[\]*_#@{}<>=\s\u00e0-\u00ff\u1ea0-\u1ef9\u0111\u0169]/)) {
			if (line.match(RE_TRIGGER)) {
	...
	RiveScript.prototype._reply_regexp = function (user, regexp) {
		...
		// change(1.1): add-backup
		regexp = regexp.replace(/_/g, "<vietnameseword>");

		// Simple replacements.
		regexp = regexp.replace(/\*/g, "(.+?)");  // Convert * into (.+?)
		regexp = regexp.replace(/#/g,  "(\\d+?)"); // Convert # into (\d+?)
		// change(1.2): delete
		//regexp = regexp.replace(/_/g,  "(\\w+?)"); // Convert _ into (\w+?) --> FAIL: [A-Za-z]
		...
		while (match) {
			...
			// change(1.3): replace
			pipes = pipes.replace(new RegExp(this.quotemeta("<vietnameseword>"), "g"), VI_ALPHABET_IGNORE);

			regexp = regexp.replace(new RegExp("\\s*\\[" + this.quotemeta(match[1]) + "\\]\\s*"),
				"(?:" + pipes + ")");
			match  = regexp.match(/\[(.+?)\]/); // Circle of life!
		}
		// change(1.4): add restore
		regexp = regexp.replace(/<vietnameseword>/g, VI_ALPHABET);
	...
	RiveScript.prototype._process_tags = function (user, msg, reply, st, bst, step, scope) {
		...
		while (match) {
			...
			//reply = reply.replace(new RegExp("\\{@\\s*?" + this.quotemeta(target) + "\\s*?\\}", "i"), subreply);
			match = reply.match(/\{@(.+?)\}/);
		}
	...
	RiveScript.prototype._substitute = function (msg, list) {
		...
		for (var i = 0, end = this._sorted["lists"][list].length; i < end; i++) {
			...
			msg = msg.replace(new RegExp("^" + qm + VI_WORD_SUBSTITUTE, "g"), result + "$1");
			msg = msg.replace(new RegExp(VI_WORD_SUBSTITUTE + qm + VI_WORD_SUBSTITUTE, "g"), "$1" + result + "$2");
			msg = msg.replace(new RegExp(VI_WORD_SUBSTITUTE + qm + "$", "g"), "$1" + result);
			//if (msg.indexOf("<rot13sub>") > -1) {
			//	msg = msg + "";
			//}
		}
	...
	RiveScript.prototype._strip_nasties = function (string) {
		// Tieng viet co dau (1): \u00c0-\u00ff
		// Tieng viet co dau (2): \u1ea0-\u1ef9
		// Tieng viet co dau (Ðd): \0110\u0111
		// Phu am co dau: \uE0\uE1\uE2\uE3\uE8\uE9\uEA\uEC\uED\uF2\uF3\uF4\uF5\uF9\uFA\u103\u111\u129\u169\u1A1\u1B0\u103\u1EA1\u1EA3\u1EA5\u1EA7\u1EA9\u1EAB\u1EAD\u1EAF\u1EB1\u1EB3\u1EB5\u1EB7\u1EB9\u1EBB\u1EBD\u1EC1\u1EC1\u1EC3\u1EC5\u1EC7\u1EC9\u1ECB\u1ECD\u1ECF\u1ED1\u1ED3\u1ED5\u1ED7\u1ED9\u1EDB\u1EDD\u1EDF\u1EE1\u1EE3\u1EE5\u1EE7\u1EE9\u1EEB\u1EED\u1EEF\u1EF1\u1EF3\u1EF5\u1EF7\u1EF9
		string = string.replace(RE_ALPHABET_SUBSTITUTE, "");
		return string;
	};
